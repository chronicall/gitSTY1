...
void eval(char *cmdline) 
{
    ...
    // If command is not built-in, run it here.
    if(!builtin_cmd(argv)) {
        Sigemptyset(&mask);
        ...
        if((pid = Fork()) == 0) {
            ...
            // Create a new process group with ID equal to the pid of the child.
            Setpgid(0, 0);

            // Run the program with execve.
        ...
        // If not a background command, wait until the foreground process terminates
        if(!bg) {
...
void waitfg(pid_t pid)
{
    // Get the job with pid, while it's state is FG, loop.
    //int jstate;

    //jstate = getjobpid(jobs, pid)->state;

    // Loop while pid is the fg pid.
...
void sigchld_handler(int sig) 
{
    ...
    while((pid = waitpid(-1, &status, WUNTRACED|WNOHANG)) > 0) {
        ...
        if(WIFSTOPPED(status)) {
            // If stopped, change job state to stopped.
        }
...
void sigint_handler(int sig) 
{
    ...
    // Get pid of foreground process.
    pid = fgpid(jobs);
    if(pid == 0) // No foreground job.
        return;
...
void sigtstp_handler(int sig) 
{
    ...
    // Send SIGSTP to the process group.
    Kill(-pgid, SIGTSTP);
